    ...
    for airport in dico_airports:
        ...
        for (runway_pol,runway_start,runway_end,runway_width) in apt['runway'][1]+apt['runway'][2]:
            ...
            for pol in refactor_multipol_2_list:
                ...
                pols.append(pol)
        for pol in pols:
            refactor_multipol_3 = VECT.ensure_MultiPolygon(pol.difference(ops.unary_union([pol2 for pol2 in pols if pol2!=pol])))
            ...
            refactor_multipol_4 = VECT.ensure_MultiPolygon(pol.intersection(ops.unary_union([pol2 for pol2 in pols if pol2!=pol])))
...
def flatten_helipads(airport_layer,vector_map,tile, treated_area):
    ...
    for nodeid in (x for x in airport_layer.dicosmn if x in airport_layer.dicosmtags['n'] and 'aeroway' in airport_layer.dicosmtags['n'][x] and airport_layer.dicosmtags['n'][x]['aeroway']=='helipad'):
        ...
        #seeds.append(center)
        total+=1
    helipad_area_refactor_6=VECT.ensure_MultiPolygon(VECT.cut_to_tile(ops.unary_union(multipol)))
